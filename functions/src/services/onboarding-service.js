...
function onboardingService(db,{Timestamp,FieldValue,professionalLimits}){
  async function complete({uid,email,input={}}){
    ...
    const businessId=`biz_${authUid}`,membershipId=`${businessId}_${authUid}`,
      refs={profile:db.doc(`users/${authUid}`),business:db.doc(`businesses/${businessId}`),member:db.doc(`businesses/${businessId}/members/${authUid}`),membership:db.doc(`memberships/${membershipId}`),onboarding:db.doc(`onboarding/${authUid}`),settingsDefault:db.doc(`businesses/${businessId}/settings/default`),settingsOperation:db.doc(`businesses/${businessId}/settings/operation`),audit:db.doc(`businesses/${businessId}/auditLogs/account_created_${authUid}`)};
    return db.runTransaction(async transaction=>{
      const [profileSnapshot,businessSnapshot,memberSnapshot,membershipSnapshot,onboardingSnapshot,defaultSnapshot,operationSnapshot]=await Promise.all([transaction.get(refs.profile),transaction.get(refs.business),transaction.get(refs.member),transaction.get(refs.membership),transaction.get(refs.onboarding),transaction.get(refs.settingsDefault),transaction.get(refs.settingsOperation)]),
      ...
      transaction.set(refs.member,{uid:authUid,name:profile.name||ownerName,email:authEmail,role:'owner',status:'active',spaceAccess:'all',allowedSpaceIds:[],permissions:ownerPermissions,createdAt:memberSnapshot.data()?.createdAt||createdAt,createdBy:authUid,updatedAt:FieldValue.serverTimestamp(),schemaVersion:1},{merge:true});
      ...
      return{businessId,status:'completed',created:{profile:!profileSnapshot.exists,business:!businessSnapshot.exists,member:!memberSnapshot.exists,membership:!membershipSnapshot.exists},trial:{startedAt:trialStartedAt.toDate().toISOString(),endsAt:trialEndsAt.toDate().toISOString(),preserved:Boolean(priorTrialStart&&priorTrialEnd)}};
    });
  }
  return{complete};
}
