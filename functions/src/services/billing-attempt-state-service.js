...
function canonicalAttemptStatus({providerStatus,paymentStatus,statusDetail,currentStatus}={}){
...
function attemptStatePatch({currentStatus,providerStatus,paymentStatus,statusDetail,now=new Date().toISOString(),source='provider'}={}){
  const classified=canonicalAttemptStatus({providerStatus,paymentStatus,statusDetail,currentStatus}),transition=transitionAttempt(currentStatus,classified.status),status=transition.status,terminal=TERMINAL_ATTEMPT_STATUSES.has(status);
