...
exports.submitCatalogOrder=onCall(CATALOG_OPTIONS,async request=>{
  try{
    ...
    const catalogItems=new Map((context.catalog.items||[]).map(item=>[String(item.id),item])),items=[];for(const raw of requested){const item=catalogItems.get(String(raw.catalogItemId||'')),quantity=Number(raw.quantity);if(!item||item.active===false||!Number.isInteger(quantity)||quantity<1||quantity>999)throw new HttpsError('invalid-argument','Um produto do pedido não está disponível.');const stock=Number(item.availableQuantity??0)-Number(item.reservedStock??0),tracks=item.controlaEstoque!==false;if(tracks&&context.catalog.stockBehavior!=='allow_negative'&&context.catalog.stockBehavior!=='preorder'&&quantity>Math.max(0,stock))throw new HttpsError('failed-precondition',`${item.productName} não possui estoque suficiente.`);const unitPrice=Number(Number(item.salePrice||0).toFixed(2)),subtotal=Number((unitPrice*quantity).toFixed(2));if(!(unitPrice>0))throw new HttpsError('failed-precondition','Produto sem preço válido.');items.push({productId:String(item.productId||''),catalogItemId:String(item.id),name:String(item.productName||'Produto').slice(0,160),image:String(item.productImage||''),quantity,unitPrice,subtotal})}
