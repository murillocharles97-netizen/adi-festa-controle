...
test('31 Pix expirado revalida o código em vez de consumir a cotação antiga',()=>{const source=fs.readFileSync(path.join(__dirname,'../src/services/coupon-firestore-service.js'),'utf8');assert.match(source,/reserveCheckoutCoupon/);assert.match(source,/error\.code\s*!==\s*["']quote_expired["']/);assert.match(source,/validateAndQuote\([\s\S]*?code:\s*normalizedCode/)});
test('32 checkout Pix não grava campos undefined e participa do mesmo batch',async()=>{const writes=[],db={doc:value=>({path:value,set:async(data,options)=>writes.push({data,options})})},writer={set:(ref,data,options)=>writes.push({ref,data,options})},service=couponFirestoreService(db);await service.markCheckout({redemptionId:'redemption_pix',internalSubscriptionId:'operation_pix',providerOrderId:'order_pix',writer});assert.equal(writes.length,1);assert.equal(writes[0].ref.path,'couponRedemptions/redemption_pix');assert.equal(writes[0].data.mercadoPagoSubscriptionId,null);assert.equal(writes[0].data.mercadoPagoPaymentId,null);assert.equal(writes[0].data.internalSubscriptionId,'operation_pix');assert.equal(Object.values(writes[0].data).includes(undefined),false);assert.deepEqual(writes[0].options,{merge:true})});
