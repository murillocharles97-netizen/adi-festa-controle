...
async function main(){
  ...
  try{
    ...
    assert.ok(reconciliation.checked>=1);assert.equal((reconciliation.reasons.created||0)+(reconciliation.reasons['already-created']||0)>=1,true);assert.equal(beatriz.amountCents,6350);assert.equal(beatriz.paymentMethod,'pix');assert.equal(beatriz.autoGenerated,true);
    ...
    const buggyFirst=await service.projectPayment(businessId,buggyPaymentId,(await db.doc(`businesses/${businessId}/payments/${buggyPaymentId}`).get()).data()),buggyRetry=await service.projectPayment(businessId,buggyPaymentId,(await db.doc(`businesses/${businessId}/payments/${buggyPaymentId}`).get()).data()),buggyEvidence=(await db.doc(`businesses/${businessId}/balanceEvents/payment_received:${buggyPaymentId}`).get()).data(),buggyApplied=(await db.doc(`businesses/${businessId}/payments/${buggyPaymentId}`).get()).data(),buggyEntry=await db.doc(`financialSpaces/${spaceId}/entries/credit_payment_${buggyPaymentId}`).get();
    assert.equal([buggyFirst,buggyRetry].filter(result=>result.created===true).length<=1,true);assert.equal(buggyRetry.created,false);assert.equal(buggyEntry.exists,true);assert.equal(buggyEntry.data().amountCents,6350);assert.equal(buggyEvidence.status,'applied_by_reconciliation');assert.equal(buggyEvidence.amount,63.5);assert.equal(buggyApplied.applicationStatus,'applied');assert.equal(buggyApplied.financialOperationId,`payment_received:${buggyPaymentId}`);
