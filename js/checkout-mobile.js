(function(){
  ...
  function recentProducts(){const ids=[],spaceId=String(window.SpaceContext?.salesId?.()||''),knownIds=new Set((window.SpaceContext?.list?.()||[]).map(space=>space.id)),sales=[...(DB.carregar().vendas||[])].filter(sale=>(!window.SpaceEngine?.isValidSale||window.SpaceEngine.isValidSale(sale))&&(!window.SpaceEngine?.saleBelongsTo||!spaceId||window.SpaceEngine.saleBelongsTo(sale,spaceId,knownIds)));for(let saleIndex=sales.length-1;saleIndex>=0&&ids.length<8;saleIndex--){for(const item of sales[saleIndex].itens||[]){if(item.produtoId&&!ids.includes(item.produtoId))ids.push(item.produtoId);if(ids.length>=8)break}}return ids.map(product).filter(item=>item&&item.ativo!==false&&(!window.SpaceEngine?.productAllowsSpace||window.SpaceEngine.productAllowsSpace(item,spaceId)))}
  function mobileHeader(page){
    if($('.mobile-sale-filters',page))return;
    const tools=$('.pos-tools',page),recent=recentProducts();
    tools?.insertAdjacentHTML('afterend',`<div class="mobile-sale-filters"><button class="active" data-sale-filter="quick">${icon('zap')} Rápidos</button><button data-sale-filter="favorites">${icon('star')} Favoritos</button><button data-sale-filter="sold">${icon('flame')} Mais vendidos</button><button data-sale-filter="low">${icon('triangle-alert')} Baixo estoque</button></div>${recent.length?`<section class="mobile-recent-products"><header><b>Últimos vendidos</b></header><div>${recent.map(item=>{const recurringNoStock=item.productType==='recurring'&&!(window.productControlsStock?.(item)??(!item.semControleEstoque&&item.controlaEstoque!==false));return `<button data-recent-product="${item.id}">${window.ProductImages?.markup?.(item,{className:'sale-recent-photo'})||`<span>${esc(item.nome.slice(0,2).toUpperCase())}</span>`}<b>${esc(item.nome)}</b><small>${money(item.preco)}</small>${recurringNoStock?`<em>${icon('calendar-clock')}${esc(window.getProductRenewalPeriod?.(item)||'30 dias')}</em>`:''}</button>`}).join('')}</div></section>`:''}<div class="mobile-products-title"><b>Produtos</b><small>Toque para adicionar · segure para opções</small></div>`);
  }
  function decorateProducts(page){
    $$('.pos-product',page).forEach(card=>{
      ...
      if(!$('.mobile-sale-favorite',card))card.insertAdjacentHTML('beforeend',`<span class="mobile-sale-favorite ${item.favorito?'active':''}" data-fav="${item.id}" role="button" tabindex="0" aria-label="${item.favorito?'Remover dos favoritos':'Adicionar aos favoritos'}" aria-pressed="${Boolean(item.favorito)}">${icon('star')}</span>`);
    });
  }
  ...
  function enhanceCart(summary){
    ...
    const total=$('.total-row b',summary)?.textContent||money(0),finish=$('#finish-sale',summary),finishState=finish?.dataset.saleState||'normal';if(finish&&finishState==='normal'&&!$('strong',finish))finish.innerHTML=`${icon('check')} Concluir venda <strong>${total}</strong>`;else if(finish&&finishState==='normal'&&$('strong',finish)?.textContent!==total)$('strong',finish).textContent=total;
  ...
  function openSummary(){const summary=$('#pos-summary'),overlay=$('[data-sale-cart-overlay]')||$('.pos-summary-overlay'),trigger=$('#open-sale-summary');if(!summary)return false;summary.hidden=false;if(overlay)overlay.hidden=false;summary.classList.add('mobile-open');summary.setAttribute('aria-hidden','false');trigger?.setAttribute('aria-expanded','true');document.body.classList.add('sale-sheet-open');overlay?.classList.add('open');enhanceCart(summary);setTimeout(()=>$('#close-sale-summary',summary)?.focus(),0);return true}
  function closeSummary({immediate=false}={}){const summary=$('#pos-summary'),overlay=$('[data-sale-cart-overlay]')||$('.pos-summary-overlay'),trigger=$('#open-sale-summary');summary?.classList.remove('mobile-open','expanded','discount-open','note-open');summary?.setAttribute('aria-hidden','true');trigger?.setAttribute('aria-expanded','false');document.body.classList.remove('sale-sheet-open');overlay?.classList.remove('open');const finish=()=>{if(summary&&!summary.classList.contains('mobile-open'))summary.hidden=true;if(overlay&&!overlay.classList.contains('open'))overlay.hidden=true};if(immediate)finish();else setTimeout(finish,180);trigger?.focus({preventScroll:true});return Boolean(summary)}
  function updatePayment(method){payment=method;window.CheckoutPaymentMethod=method;const summary=$('#pos-summary'),status=$('#sale-status',summary),select=$('#sale-payment-method',summary);if(select)select.value=method;if(status){status.value=method==='fiado'?'fiado':'pago';status.dispatchEvent(new Event('change',{bubbles:true}))}$$('[data-payment]',summary).forEach(button=>button.classList.toggle('active',button.dataset.payment===method));const note=$('.mobile-payment-note',summary);if(note)note.innerHTML=method==='fiado'?`${icon('hand-coins')}<span><b>Venda fiado</b><small>O valor será somado à conta do cliente.</small></span>`:method==='cartao_presencial'?`${icon('contactless')}<span><b>Cartão na maquininha</b><small>A venda só finaliza após a aprovação.</small></span>`:`${icon('circle-check')}<span><b>${method==='pix'?'Pagamento via Pix':method==='dinheiro'?'Pagamento em dinheiro':'Pagamento no cartão'}</b><small>Valor será recebido agora.</small></span>`;enhanceCart(summary);window.lucide?.createIcons()}
  function menu(item){
    const root=$('#modal'),controlsStock=window.productControlsStock?.(item)??(item.itemKind!=='service'&&!item.semControleEstoque&&item.controlaEstoque!==false);root.innerHTML=`<div class="modal-bg mobile-product-menu-bg"><section class="mobile-product-menu"><span class="sheet-handle"></span><header>${window.ProductImages?.markup?.(item,{className:'sale-menu-photo'})||`<span>${esc(item.nome.slice(0,2).toUpperCase())}</span>`}<div><h3>${esc(item.nome)}</h3><p>${money(item.preco)}${item.itemKind==='service'?' · Serviço':''}</p></div><button class="close">${icon('x')}</button></header><button data-menu-add-five>${icon('plus')} Adicionar 5 à sacola</button><button data-menu-favorite>${icon('star')} ${item.favorito?'Remover dos favoritos':'Favoritar item'}</button><button data-menu-edit>${icon('pencil')} Editar item</button>${controlsStock?`<button data-menu-stock>${icon('package-plus')} Entrada de estoque</button><button data-menu-history>${icon('history')} Histórico de estoque</button>`:''}</section></div>`;
    const close=()=>root.innerHTML='';$('.close',root).onclick=close;$('[data-menu-add-five]',root).onclick=()=>{close();const card=$(`.pos-product[data-add="${CSS.escape(item.id)}"]`);for(let n=0;n<5;n++)card?.click()};$('[data-menu-favorite]',root).onclick=()=>{close();$(`[data-fav="${CSS.escape(item.id)}"]`)?.click()};$('[data-menu-edit]',root).onclick=()=>{close();window.ProdutosMobile?.productForm(item.id)};$('[data-menu-stock]',root)?.addEventListener('click',()=>{close();window.ProdutosMobile?.stockEntry(item.id)});$('[data-menu-history]',root)?.addEventListener('click',()=>{close();window.ProdutosMobile?.history(item.id)});window.lucide?.createIcons()
  }
  function bindProductGestures(page){
    ...
    grid.addEventListener('pointerup',()=>clearTimeout(longPress));
  ...
  function bindPage(page){
    ...
    if(!$('.pos-summary-overlay',page))page.insertAdjacentHTML('beforeend','<div class="pos-summary-overlay" data-sale-cart-overlay hidden></div>');
    page.addEventListener('click',event=>{
      ...
      const recent=event.target.closest('[data-recent-product]');if(recent){const item=product(recent.dataset.recentProduct);if(!item)return;if(item.productType==='recurring'){Checkout.openRecurringProduct?.(item,recent);return}if(ProductVariations.isVariable(item)){Checkout.openVariantPicker(item,null,recent);return}if(getProductStockStatus(item)==='esgotado'&&(window.productControlsStock?.(item)??(!item.semControleEstoque&&item.controlaEstoque!==false))&&!item.allowNegativeStock){Utils.toast('Produto sem estoque.',true);return}Checkout.addSaleItem(ProductVariations.saleItem(item,null,1),{source:recent});return}
      const favorite=event.target.closest('[data-fav]');if(favorite)setTimeout(()=>{const item=product(favorite.dataset.fav);favorite.classList.toggle('active',Boolean(item?.favorito));favorite.innerHTML=icon('star');favorite.setAttribute('aria-label',item?.favorito?'Remover dos favoritos':'Adicionar aos favoritos');favorite.setAttribute('aria-pressed',String(Boolean(item?.favorito)));window.lucide?.createIcons()},0);
    ...
    page.addEventListener('keydown',event=>{if(event.key==='Escape'&&$('#pos-summary')?.classList.contains('mobile-open')){event.preventDefault();closeSummary();return}const favorite=event.target.closest('[data-fav]');if(favorite&&['Enter',' '].includes(event.key)){event.preventDefault();favorite.click()}});
  ...
  function enhance(){
    if(enhancing||!mq.matches||Router.atual()!=='vender')return;const page=$('.pos-page');if(!page)return;const search=$('#product-search',page);if(search){search.placeholder='Buscar produto ou código';search.setAttribute('aria-label','Buscar produto por nome, código de barras ou categoria')}const first=!page.dataset.mobileSaleInitialized;if(first){payment=$('#sale-payment-method',page)?.value||window.CheckoutPaymentMethod||'pix';window.CheckoutPaymentMethod=payment}enhancing=true;try{page.classList.add('mobile-sale-page');mobileHeader(page);decorateProducts(page);const summary=$('#pos-summary',page);if(summary){summaryTools(summary);$$('[data-payment]',summary).forEach(button=>button.classList.toggle('active',button.dataset.payment===payment));enhanceCart(summary)}bindPage(page);applyPending(page);if(first){page.dataset.mobileSaleInitialized='1';window.lucide?.createIcons()}}finally{enhancing=false}
  }
  ...
  addEventListener('sale-products-filtered',()=>{if(!mq.matches||Router.atual()!=='vender')return;const grid=$('#pos-grid');grid?.querySelector('.mobile-sale-empty')?.remove();if(!$$('.pos-product:not([hidden])',grid).length)grid?.insertAdjacentHTML('beforeend','<div class="mobile-sale-empty">Nenhum produto encontrado.</div>')});
  ...
  addEventListener('hashchange',()=>{if($('#pos-summary')?.classList.contains('mobile-open'))closeSummary({immediate:true});else document.body.classList.remove('sale-sheet-open');setTimeout(enhance,0)});
