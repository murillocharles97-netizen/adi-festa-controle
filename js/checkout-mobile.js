(function(){
  ...
  function recentProducts(){const ids=[],sales=[...(DB.carregar().vendas||[])];for(let saleIndex=sales.length-1;saleIndex>=0&&ids.length<8;saleIndex--){for(const item of sales[saleIndex].itens||[]){if(item.produtoId&&!ids.includes(item.produtoId))ids.push(item.produtoId);if(ids.length>=8)break}}return ids.map(product).filter(item=>item&&item.ativo!==false)}
