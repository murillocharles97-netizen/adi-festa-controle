(function(){
  ...
  function mobileHeader(page){
    ...
    tools?.insertAdjacentHTML('afterend',`<div class="mobile-sale-filters"><button class="active" data-sale-filter="quick">${icon('zap')} Rápidos</button><button data-sale-filter="favorites">${icon('star')} Favoritos</button><button data-sale-filter="sold">${icon('flame')} Mais vendidos</button><button data-sale-filter="low">${icon('triangle-alert')} Baixo estoque</button></div>${recent.length?`<section class="mobile-recent-products"><header><b>Últimos vendidos</b><button type="button" data-sale-filter="sold">Ver todos</button></header><div>${recent.map(item=>`<button data-recent-product="${item.id}">${window.ProductImages?.markup?.(item,{className:'sale-recent-photo'})||`<span>${esc(item.nome.slice(0,2).toUpperCase())}</span>`}<b>${esc(item.nome)}</b><small>${money(item.preco)}</small></button>`).join('')}</div></section>`:''}<div class="mobile-products-title"><b>Produtos</b><small>Toque para adicionar · segure para opções</small></div>`);
  }
  function decorateProducts(page){
    $$('.pos-product',page).forEach(card=>{
      ...
      if(!$('.mobile-sale-add-glyph',card))card.insertAdjacentHTML('beforeend',`<span class="mobile-sale-add-glyph" aria-hidden="true">${icon('plus')}</span>`);
    });
  }
  ...
  function menu(item){
    ...
    window.MobileMotion?.sheet($('.mobile-product-menu',root));
  ...
  addEventListener('sale-item-added',event=>{if(!mq.matches||Router.atual()!=='vender')return;const source=event.detail?.source?.isConnected?event.detail.source:$(`.pos-product[data-add="${CSS.escape(event.detail?.item?.produtoId||'')}"]`),bag=$('#open-sale-summary');window.MobileMotion?.feedback(source,'success');if(event.detail?.first)window.MobileMotion?.fly(source,bag,{fallback:String(event.detail?.item?.nome||'').slice(0,2).toUpperCase()});if(event.detail?.variable)Utils.toast('✓ Item adicionado')});
  addEventListener('sale-products-filtered',event=>{if(!mq.matches||Router.atual()!=='vender')return;const grid=$('#pos-grid');grid?.classList.add('motion-filtering');grid?.querySelector('.mobile-sale-empty')?.remove();const visible=$$('.pos-product:not([hidden])',grid);if(!visible.length)grid?.insertAdjacentHTML('beforeend','<div class="mobile-sale-empty">Nenhum produto encontrado.</div>');visible.forEach((card,index)=>window.MobileMotion?.animate(card,[{opacity:0,transform:'translateY(7px)'},{opacity:1,transform:'translateY(0)'}],{duration:170,delay:Math.min(index,8)*10}));setTimeout(()=>grid?.classList.remove('motion-filtering'),230)});
  ...
  new MutationObserver(records=>{if(!mq.matches)return;records.forEach(record=>record.addedNodes.forEach(node=>{if(!(node instanceof Element))return;const sheet=node.matches?.('.variation-picker')?node:node.querySelector?.('.variation-picker');if(sheet&&!sheet.dataset.motionMounted){sheet.dataset.motionMounted='true';window.MobileMotion?.sheet(sheet)}}))}).observe($('#modal'),{childList:true,subtree:true});
