(function(){
  ...
  const isValidSale=sale=>sale&&sale.ativo!==false&&sale.active!==false&&sale.desfeita!==true&&!sale.deletedAt&&!INVALID_SALE_STATUSES.has(normalizedStatus(sale.status||sale.saleStatus||sale.tipo));
  ...
  const saleDebt=sale=>normalizedStatus(sale?.status)!=='fiado'||sale?.creditSettled===true?0:Math.max(0,Number(sale.creditRemainingAmount??sale.creditOriginalAmount??sale.valorFinal??sale.valorTotal??0));
  ...
  function model(){
    const source=DB.carregar(),db=window.SpaceContext?.contextualData?.(source,'home')||source,selectionId=db.spaceSelectionId||window.SpaceContext?.ALL_SPACES||'all_spaces',isAllSpaces=selectionId===(window.SpaceContext?.ALL_SPACES||'all_spaces'),now=new Date(),todayKey=dayKey(now),yesterday=new Date(now);yesterday.setDate(yesterday.getDate()-1);
    const sales=(db.vendas||[]).filter(isValidSale),today=sales.filter(sale=>dayKey(sale.data||sale.createdAt)===todayKey),yesterdaySales=sales.filter(sale=>dayKey(sale.data||sale.createdAt)===dayKey(yesterday));
    ...
    const items=today.reduce((sum,sale)=>sum+(sale.itens||[]).reduce((subtotal,item)=>subtotal+Number(item.quantidade??item.quantity??0),0),0);
    const customers=new Set(today.map(sale=>sale.clienteId||sale.clientId).filter(Boolean)).size;
    const validClientIds=isAllSpaces?null:new Set(sales.map(sale=>sale.clienteId||sale.clientId).filter(Boolean).map(String)),scopedClients=(db.clientes||[]).filter(client=>client.ativo!==false&&(isAllSpaces||validClientIds.has(String(client.id)))),newCustomers=scopedClients.filter(client=>dayKey(client.criadoEm||client.createdAt)===todayKey).length;
    const scopedDebtByClient=isAllSpaces?null:sales.reduce((map,sale)=>{const id=sale.clienteId||sale.clientId,amount=saleDebt(sale);if(id&&amount)map.set(String(id),(map.get(String(id))||0)+amount);return map},new Map()),debtors=scopedClients.filter(client=>isAllSpaces?Number(client.saldo)<0:(scopedDebtByClient?.get(String(client.id))||0)>0),debt=isAllSpaces?debtors.reduce((sum,client)=>sum+Math.abs(Number(client.saldo)),0):[...(scopedDebtByClient?.values()||[])].reduce((sum,value)=>sum+value,0);
    ...
    const localRenewals=window.CustomerSubscriptions?.metrics?.(now.toISOString())||{dueToday:0,due7:0,forecastValue:0},renewals=isAllSpaces?(renewalCache.value||localRenewals):{dueToday:0,due7:0,forecastValue:0};
    ...
    const goal=Number(window.SpaceContext?.goalFor?.(db.config||{},selectionId)??db.config?.dashboard?.dailySalesGoal??db.config?.dailySalesGoal??0),goalPercent=goal>0?sold/goal*100:0;
    const week=[];for(let offset=6;offset>=0;offset--){const date=new Date(now);date.setDate(date.getDate()-offset);const key=dayKey(date);week.push({date,key,label:date.toLocaleDateString('pt-BR',{weekday:'short'}).replace('.',''),value:sales.filter(sale=>dayKey(sale.data||sale.createdAt)===key).reduce((sum,sale)=>sum+saleValue(sale),0),today:offset===0})}
    const name=String(window.FirebaseSession?.profile?.name||db.config?.responsavel||'').trim().split(/\s+/)[0]||'Murillo';
    return{db,now,sales,today,sold,cost,profit,margin:sold>0?profit/sold*100:0,soldYesterday,profitYesterday,items,customers,newCustomers,debtors,debt,clients:scopedClients,products,out,low,renewals,pendingOrders,goal,goalPercent,week,name,selectionId,isAllSpaces,unassignedLegacySales:Number(db.unassignedLegacySales||0)};
  }
  ...
  function goalCard(data){
    if(!data.goal)return`<button class="home-goal-card empty-goal" type="button" data-home-goal>${icon('target')}<span><b>${data.isAllSpaces?'Defina sua meta geral':'Defina a meta deste espaço'}</b><small>${data.isAllSpaces?'Ela é independente das metas por espaço.':'Somente as vendas deste espaço entram no progresso.'}</small></span><em>Configurar ${icon('chevron-right')}</em></button>`;
  ...
  function render(){
    ...
    return`<section class="mobile-home-dashboard">${contextBar}<div class="home-greeting"><h2>${icon('hand')} ${greeting()}, ${esc(data.name)}!</h2><p>${esc(smartMessage(data))}</p></div>${legacyNotice(data)}${goalCard(data)}${main}${secondary}${attention}${chart(data)}${quick}</section>`
  }
  function goalModal(){
  ...
  async function loadRenewalAttention(){
    ...
    if(window.SpaceContext?.homeId?.()&&window.SpaceContext.homeId()!==window.SpaceContext.ALL_SPACES)return;
