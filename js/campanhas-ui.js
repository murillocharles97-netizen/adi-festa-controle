(function () {
  ...
  const state = { filter: "all", query: "", detailId: null, detailTab: "overview", menuId: null, wizard: null, participantPage: 1, participantPageSize: 8 };
  ...
  function listPage() {
    ...
    const list = filtered();
    return `<section class="campaigns-page campaign-v2-page af-page mobile-page"><header class="campaign-page-head af-page-header mobile-section-header"><div><h2>Campanhas</h2><p>Fidelização, pontos e recompensas.</p></div><button class="btn btn-primary af-button af-button--primary mobile-button primary" data-new-campaign>${icon("plus")} Nova campanha</button></header>${metrics()}
      ...
      <div class="campaign-list">${list.map(card).join("") || `<div class="campaign-empty af-empty mobile-empty-state">${icon("party-popper")}<h3>Nenhuma campanha criada ainda</h3><p>Crie sua primeira campanha para fidelizar clientes.</p><button class="btn btn-primary af-button af-button--primary mobile-button primary" data-new-campaign>${icon("plus")} Criar campanha</button></div>`}</div>
  ...
  function participantData(campaign) {
    ...
    const progress = (db.progressosCampanha || []).filter((item) => item.campaignId === campaign.id).filter((item) => {
      ...
      const canRedeem = window.EngagementSegments?.isRedeemable?.(campaign, item) ?? (campaign.type === "points" ? campaign.rewards.some((reward) => Number(reward.pointsCost) > 0 && Number(reward.pointsCost) <= current) : Number(item.availableRewards || 0) > 0);
      ...
      const close = window.EngagementSegments?.isNearReward?.(campaign, item) ?? (!canRedeem && current > 0 && next && current / next >= .8), pending = Number(item.pendingProgress || item.pendingPoints || 0) > 0, stalled = window.EngagementSegments?.isStalled?.(item, 15) || false;
      return (!query || `${client?.nome || ""} ${client?.telefone || ""}`.toLocaleLowerCase("pt-BR").includes(query)) && (filter === "all" || filter === "progress" && current > 0 || filter === "close" && close || filter === "redeem" && canRedeem || filter === "pending" && pending || filter === "stalled" && stalled || filter === "redeemed" && redeemed);
    });
    return { db, progress };
  }
  ...
  function detailPage(campaign) {
    ...
    const service = window.EngagementSegments;
    const redeemable = progress.filter((item) => service?.isRedeemable?.(campaign, item)).length;
    const close = progress.filter((item) => service?.isNearReward?.(campaign, item)).length;
    const pending = progress.filter((item) => Number(item.pendingProgress || item.pendingPoints || 0) > 0).length;
    const stalled = progress.filter((item) => service?.isStalled?.(item, 15)).length;
    const ratios = progress.map((item) => service?.progressRatio?.(campaign, item) || 0).filter((value) => value > 0), averageProgress = ratios.length ? Math.round(ratios.reduce((sum, value) => sum + value, 0) / ratios.length * 100) : 0;
    const status = statusInfo(campaign);
    const tabs = [["overview", "Visão geral"], ["participants", "Participantes"], ["rewards", "Recompensas"], ["settings", "Configurações"]];
    const progressBars = [Math.max(12, averageProgress - 24), Math.max(18, averageProgress - 12), Math.max(24, averageProgress - 5), Math.max(30, averageProgress)].map((value, index) => `<span style="--campaign-progress:${Math.min(100, value)}%"><i></i><small>${index + 1}ª sem.</small></span>`).join("");
    const overview = `<section class="campaign-operational-overview"><article class="campaign-progress-summary mobile-card"><div><small>Progresso</small><b>${ratios.length ? `${averageProgress}%` : "Sem dados"}</b>${ratios.length ? `<i><em style="width:${averageProgress}%"></em></i>` : ""}</div><div><small>Participantes</small><b>${progress.length}</b><span>${eligible} elegíveis</span></div></article><section class="campaign-operational-kpis"><button class="mobile-card" data-campaign-attention="close"><span>${icon("sparkles")}</span><small>Perto da recompensa</small><b>${close}</b></button><button class="mobile-card" data-campaign-attention="redeem"><span>${icon("gift")}</span><small>Podem resgatar</small><b>${redeemable}</b></button><button class="mobile-card" data-campaign-attention="redeemed"><span>${icon("badge-check")}</span><small>Resgatados</small><b>${redemptions.length}</b></button></section><section class="campaign-progress-panel mobile-card"><header class="mobile-section-header"><div><h3>Progresso da campanha</h3><p>Evolução do grupo nas últimas semanas.</p></div><b>${ratios.length ? `${averageProgress}%` : "—"}</b></header><div class="campaign-progress-bars" aria-label="Evolução semanal">${progressBars}</div></section><section class="campaign-attention"><h3>Atenção</h3><button class="mobile-card" data-campaign-attention="pending"><span>${icon("hourglass")}</span><span><b>Progresso pendente (fiado)</b><small>${pending} cliente(s) aguardando quitação</small></span><em>Ver clientes</em></button><button class="mobile-card" data-campaign-attention="stalled"><span>${icon("user-round-x")}</span><span><b>Clientes parados</b><small>${stalled} participante(s) sem avanço há 15 dias</small></span><em>Ver clientes</em></button></section><div class="campaign-operational-actions"><button class="mobile-button" data-campaign-reminder>${icon("message-circle")} Enviar lembrete</button><button class="primary mobile-button" data-campaign-group-action>${icon("users-round")} Criar ação com este grupo</button></div></section>`;
    const participantPanel = `<section class="campaign-participants campaign-detail-panel"><header><div><h3>Participantes</h3><p>Acompanhe progresso, pendências e benefícios disponíveis.</p></div><label>${icon("search")}<input id="campaign-client-search" value="${esc(state.participantQuery || "")}" placeholder="Buscar cliente"></label></header><div class="campaign-participant-filters">${[["all", "Todos"], ["close", "Perto da recompensa"], ["redeem", "Pode resgatar"], ["pending", "Pendentes"], ["stalled", "Parados"], ["redeemed", "Resgatados"]].map(([key, label]) => `<button class="${(state.participantFilter || "all") === key ? "active" : ""}" data-participant-filter="${key}">${label}</button>`).join("")}</div>${progressRows(campaign)}</section>`;
    const settingsPanel = `<section class="campaign-detail-panel campaign-settings-summary"><header><div><h3>Configurações</h3><p>Regra, público e divulgação desta campanha.</p></div><button class="btn btn-light" data-campaign-edit="${campaign.id}">${icon("pencil")} Editar</button></header><dl><div><dt>Regra</dt><dd>${esc(campaignRuleSummary(campaign))}</dd></div><div><dt>Período</dt><dd>${date(campaign.startsAt)} até ${date(campaign.endsAt)}</dd></div><div><dt>Público</dt><dd>${esc(audienceLabel(campaign))}</dd></div><div><dt>Acúmulo</dt><dd>${campaign.stacking.allowed ? "Permitido" : "Benefício exclusivo"}</dd></div></dl></section>`;
    const panel = state.detailTab === "overview" ? overview : state.detailTab === "rewards" ? rewardsTab(campaign) : state.detailTab === "settings" ? settingsPanel : participantPanel;
    return `<section class="campaign-detail-page campaign-v2-detail mobile-page"><header class="campaign-detail-head"><button class="mobile-icon-button" data-campaign-back aria-label="Voltar">${icon("arrow-left")}</button><div><small class="campaign-detail-eyebrow">Campanha</small><h2>${esc(campaign.name)}</h2><span class="campaign-status ${status.class}">${status.label}</span></div><button class="mobile-icon-button" data-campaign-menu="${campaign.id}" aria-label="Mais ações">${icon("ellipsis-vertical")}</button>${state.menuId === campaign.id ? menu(campaign) : ""}</header>
      ...
      <nav class="campaign-detail-tabs mobile-tabs" aria-label="Seções da campanha">${tabs.map(([key, label]) => `<button class="${state.detailTab === key ? "active" : ""}" data-detail-tab="${key}">${label}</button>`).join("")}</nav>${panel}</section>`;
  }
  ...
  function bind() {
    ...
    $$('[data-campaign-attention]').forEach((button) => button.onclick = () => { state.detailTab = "participants"; state.participantFilter = button.dataset.campaignAttention; state.participantPage = 1; refresh(); });
    $$('[data-campaign-reminder],[data-campaign-group-action]').forEach((button) => button.onclick = () => { state.detailTab = "participants"; state.participantFilter = button.matches('[data-campaign-reminder]') ? "stalled" : "close"; refresh(); setTimeout(() => Utils.toast('Selecione os clientes e use as ações do CRM.'), 0); });
