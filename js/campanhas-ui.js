(function () {
  ...
  function stepType(data) {
    return `<div class="campaign-wizard-pane"><h3>Qual resultado você quer alcançar?</h3><p>Escolha o tipo de campanha que melhor se encaixa no seu objetivo.</p><div class="campaign-type-grid">${Object.entries(Campanhas.TYPES).map(([key, info]) => `<button type="button" class="campaign-type-choice ${data.type === key ? "active" : ""}" data-wizard-type="${key}">${icon(info.icon)}<b>${esc(info.label)}</b><small>${esc(info.description)}</small><em>Boa para: ${esc(info.goodFor)}</em></button>`).join("")}</div><label>Nome da campanha<input name="name" maxlength="60" required value="${esc(data.name || "")}" placeholder="Ex.: Compre 5 e ganhe 1"></label><label>Descrição para o cliente<textarea name="description" maxlength="180">${esc(data.description || typeInfo(data).description)}</textarea></label></div>`;
  }
  ...
  function rewardRow(reward = {}, index = 0, points = false) {
    ...
    return `<div class="campaign-reward-row" data-reward-row><header><b>Recompensa ${index + 1}</b>${index ? `<button type="button" data-remove-reward="${index}" aria-label="Remover recompensa">${icon("trash-2")}</button>` : ""}</header><label>Origem<select name="rewardType"><option value="product" ${type === "product" ? "selected" : ""}>Produto do estoque</option><option value="external" ${type === "external" ? "selected" : ""}>Recompensa externa</option></select></label>${type === "product" ? `<label>Produto<select name="rewardProductId">${productOptions(reward.productId || "")}</select></label>${variants.length ? `<label>Variação<select name="rewardVariantId">${variantOptions(reward.productId || "", reward.variantId || variants[0]?.id || "")}</select></label>` : ""}` : `<label>Nome da recompensa<input name="rewardName" value="${esc(reward.name || "")}" placeholder="Ex.: Vale R$ 20"></label><label>Descrição opcional<input name="rewardDescription" value="${esc(reward.description || "")}" placeholder="Como o cliente recebe"></label>`}<div class="campaign-reward-numbers"><label>Quantidade<input name="rewardQuantity" type="number" min="1" value="${reward.quantity || 1}"></label>${points ? `<label>Pontos necessários<input name="rewardPointsCost" type="number" min="1" value="${reward.pointsCost || (index + 1) * 100}"></label>` : ""}</div><div class="campaign-reward-preview">${icon("gift")}<span><b>${esc(name)}${reward.variantId ? ` · ${esc(productVariants(reward.productId).find((variant) => variant.id === reward.variantId)?.displayName || "")}` : ""}</b><small>${points ? `${reward.pointsCost || (index + 1) * 100} pontos · ` : ""}${reward.quantity || 1} unidade(s)${type === "product" ? ` · Estoque atual: ${stock}` : " · Recompensa externa"}</small></span></div></div>`;
  }

  function stepReward(data) {
    if (["quantity_discount", "combo"].includes(data.type)) return `<div class="campaign-wizard-pane"><h3>O que ele ganha?</h3><div class="campaign-education">O benefício desta campanha é o preço especial configurado na etapa anterior. Ele será apresentado no carrinho para confirmação.</div></div>`;
    ...
    return `<div class="campaign-wizard-pane campaign-reward-step"><h3>O que o cliente pode ganhar?</h3><p>Escolha uma recompensa. Você poderá adicionar outras opções se quiser.</p><div class="campaign-rewards-editor">${rows}</div><button type="button" class="btn btn-light campaign-add-reward" data-add-reward>${icon("plus")} Adicionar outra recompensa</button></div>`;
  }
  ...
  function stepAudience(data) {
    ...
    return `<div class="campaign-wizard-pane campaign-audience-step"><h3>Quem participa e como divulgar?</h3><section><h4>Quem pode participar?</h4><div class="campaign-audience-options">${[["all", "users", "Todos os clientes", "Atuais e novos."], ["segment", "filter", "Segmento do CRM", "Público atualizado pelo segmento."], ["clients", "user-check", "Clientes específicos", "Seleção manual."]].map(([key, ico, title, text]) => `<label class="${audience === key ? "active" : ""}"><input type="radio" name="audienceType" value="${key}" ${audience === key ? "checked" : ""}>${icon(ico)}<span><b>${title}</b><small>${text}</small></span></label>`).join("")}</div>${conditional}</section><section><h4>Combina com outras campanhas?</h4><label class="campaign-setting-card"><span><b>Acumular com campanhas compatíveis</b><small>Ative se a mesma compra puder contar também para outras campanhas.</small></span><input type="checkbox" name="stackingAllowed" ${data.stacking?.allowed ? "checked" : ""}></label></section><section><h4>Onde divulgar?</h4>${[["catalog", "Catálogo online"], ["receipt", "Recibo"], ["whatsapp", "WhatsApp"]].map(([key, label]) => `<label class="campaign-setting-card"><span><b>${label}</b><small>${key === "catalog" ? "Mostra a campanha no portal do cliente." : key === "receipt" ? "Inclui o progresso no comprovante." : "Inclui o resumo na mensagem do recibo."}</small></span><input type="checkbox" name="publicity" value="${key}" ${publicConfig[key] ? "checked" : ""}></label>`).join("")}</section></div>`;
  }
  ...
  function renderWizard() {
    ...
    root.innerHTML = `<div class="modal-bg"><section class="modal-box modal-wide campaign-wizard campaign-wizard-v2"><header class="modal-head"><div><small>${wizard.editing ? "Editar" : "Nova"} campanha</small><h3>${["Resultado", "Participação", "Recompensa", "Público e divulgação", "Revisão"][wizard.step - 1]}</h3></div><button class="icon-btn" data-wizard-close>${icon("x")}</button></header><div class="campaign-wizard-steps">${["Objetivo", "Regra", "Prêmio", "Público", "Revisão"].map((label, index) => `<span class="${wizard.step >= index + 1 ? "active" : ""}"><b>${index + 1}</b>${label}</span>`).join("")}</div><form id="campaign-wizard-form"><input type="hidden" name="type" value="${data.type}"><div class="modal-body">${panes[wizard.step - 1](data)}</div><footer class="modal-foot">${wizard.step > 1 ? '<button type="button" class="btn btn-light" data-wizard-back>Voltar</button>' : "<span></span>"}<button class="btn btn-primary">${wizard.step === 5 ? (wizard.editing ? "Salvar alterações" : "Criar campanha") : "Próximo"}</button></footer></form></section></div>`;
