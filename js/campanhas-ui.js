(function () {
  ...
  function detailPage(campaign) {
    ...
    const overview = `<section class="campaign-operational-overview"><article class="campaign-progress-summary"><div><small>Progresso médio</small><b>${ratios.length ? `${averageProgress}%` : "Sem dados"}</b>${ratios.length ? `<i><em style="width:${averageProgress}%"></em></i>` : ""}</div><div><small>Participantes</small><b>${progress.length}</b><span>${eligible} elegíveis</span></div></article><section class="campaign-operational-kpis"><button data-campaign-attention="close"><span>${icon("sparkles")}</span><small>Perto da recompensa</small><b>${close}</b></button><button data-campaign-attention="redeem"><span>${icon("gift")}</span><small>Podem resgatar</small><b>${redeemable}</b></button><button data-campaign-attention="redeemed"><span>${icon("badge-check")}</span><small>Resgatados</small><b>${redemptions.length}</b></button></section><section class="campaign-attention"><h3>Atenção</h3><button data-campaign-attention="pending"><span>${icon("hourglass")}</span><span><b>Progresso pendente (fiado)</b><small>${pending} cliente(s) aguardando quitação</small></span><em>Ver clientes</em></button><button data-campaign-attention="stalled"><span>${icon("user-round-x")}</span><span><b>Clientes parados</b><small>${stalled} participante(s) sem avanço há 15 dias</small></span><em>Ver clientes</em></button></section><div class="campaign-operational-actions"><button data-campaign-reminder>${icon("message-circle")} Enviar lembrete</button><button class="primary" data-campaign-group-action>${icon("users-round")} Criar ação com este grupo</button></div></section>`;
