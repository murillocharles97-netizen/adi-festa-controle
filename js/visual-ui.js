...
window.VisualUI=(()=>{
  ...
  function dashboardEnhancements(){
    ...
    const head=app.querySelector('.page-head');if(head)head.insertAdjacentHTML('afterend',`<section class="dashboard-intro"><div><h2>${icon('hand')} Olá! Vamos vender hoje?</h2><p>Seu resumo rápido da Adi Festa.</p></div><button class="date-chip"><i data-lucide="calendar-days"></i>${new Date().toLocaleDateString('pt-BR')}</button></section>`);
