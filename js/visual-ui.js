...
window.VisualUI=(()=>{
  ...
  function productEnhancements(){
    ...
    cards.forEach(card=>{
      ...
      if(head&&!head.querySelector('[data-toggle-favorite]'))head.insertAdjacentHTML('beforeend',`<button class="product-favorite ${product.favorito?'is-favorite':''}" data-toggle-favorite="${id}" aria-label="${product.favorito?'Remover dos favoritos':'Adicionar aos favoritos'}" aria-pressed="${Boolean(product.favorito)}">${icon('star')}</button>`);
    });
  ...
  function dashboardEnhancements(){
    ...
    const head=app.querySelector('.page-head');if(head)head.insertAdjacentHTML('afterend',`<section class="dashboard-intro"><div><h2>${icon('hand')} Olá! Vamos vender hoje?</h2><p>Seu resumo rápido do seu negócio.</p></div><button class="date-chip"><i data-lucide="calendar-days"></i>${new Date().toLocaleDateString('pt-BR')}</button></section>`);
