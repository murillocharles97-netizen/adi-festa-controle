window.FinanceiroUI = (() => {
  ...
  function payablesMarkup(entries = [], compact = true) {
    ...
    return `<div class="financial-list ${compact ? "is-compact" : ""}">${entries.map((entry) => `
      <article class="financial-list-row" ${entry.entityType === "credit_card_invoice" ? `data-financial-invoice-id="${esc(entry.creditCardInvoiceId)}"` : `data-financial-entry-id="${esc(entry.id)}"`}>
  ...
  const creditCardMarkup = (card) => {
    ...
    return `<article class="financial-credit-card" data-financial-card-id="${esc(card.id)}">
      <header><span>${icon("credit-card")}</span><div><h3>${esc(card.name)}</h3><small>•••• ${esc(card.last4)}</small></div><em>${esc(status)}</em></header>
      <div class="financial-credit-card-total"><small>Fatura atual</small><strong>${money(invoice?.remainingCents || 0)}</strong><span>${invoice ? `Vence ${dateLabel(invoice.dueDate)}` : "Nenhuma compra lançada"}</span></div>
      <dl><div><dt>Limite</dt><dd>${money(card.limitCents || 0)}</dd></div><div><dt>Disponível</dt><dd>${money(card.availableCents || 0)}</dd></div></dl>
      <button type="button" data-financial-card-invoice="${esc(invoice?.id || "")}" ${invoiceDisabled ? "disabled" : ""}>${state.consolidated ? "Somente leitura" : "Ver fatura"} ${icon("chevron-right")}</button>
    </article>`;
  };
  ...
  function cardsMarkup(data) {
    ...
    return `${subpageHeader("Cartões", "Faturas, limite e parcelas do espaço atual", state.consolidated ? "" : `<button class="btn btn-primary" type="button" data-financial-new-card>${icon("plus")} Novo cartão</button>`)}${internalNav()}
      <section class="financial-credit-overview"><article><small>Total comprometido</small><strong>${money(data.creditCommittedCents || 0)}</strong><span>Saldo ainda devido nos cartões.</span></article><article><small>Cartões ativos</small><strong>${cards.length}</strong><span>Somente deste espaço financeiro.</span></article></section>
  ...
  async function openCreateCreditCard(onCreated = null) {
    if (state.consolidated) return Utils.toast("Escolha um espaço para cadastrar o cartão.", true);
    const accounts = await window.FinancialSpaceService.listFinancialAccounts(state.selectedSpaceId), draft = {
      step: 1, name: "", institution: "", last4: "", limit: "", closingDay: "", dueDay: "", paymentAccountId: "",
    };
    ...
    const host = modal().querySelector("[data-credit-card-wizard]"), sync = () => {
      ...
      const steps = [
        ...
        `<div class="modal-body financial-wizard-body"><h3>Conferir cartão</h3><p>Ele ficará disponível somente no espaço ${esc(selectedSpace()?.name || "atual")}.</p><article class="financial-wizard-review"><header><span>${icon("credit-card")}</span><div><b>${esc(draft.name || "Cartão")}</b><strong>•••• ${esc(draft.last4 || "0000")}</strong></div></header><dl><div><dt>Instituição</dt><dd>${esc(draft.institution || "Não informada")}</dd></div><div><dt>Limite</dt><dd>${draft.limit ? money(Engine.moneyInputToCents(draft.limit)) : "—"}</dd></div><div><dt>Fechamento</dt><dd>Dia ${esc(draft.closingDay)}</dd></div><div><dt>Vencimento</dt><dd>Dia ${esc(draft.dueDay)}</dd></div><div><dt>Conta para pagamento</dt><dd>${esc(accounts.find((item) => item.id === draft.paymentAccountId)?.name || "Escolher ao pagar")}</dd></div></dl></article><label class="financial-field"><span>Conta padrão para pagar <small>(opcional)</small></span><select name="paymentAccountId"><option value="">Escolher ao pagar</option>${accounts.map((account) => `<option value="${esc(account.id)}" ${account.id === draft.paymentAccountId ? "selected" : ""}>${esc(account.name)}</option>`).join("")}</select></label></div>`,
      ];
      host.innerHTML = `<header class="modal-head financial-wizard-head"><div><small>Passo ${draft.step} de 3</small><div class="financial-wizard-progress">${[1, 2, 3].map((step) => `<i class="${step <= draft.step ? "active" : ""}"></i>`).join("")}</div></div><button class="icon-btn" type="button" data-financial-close>${icon("x")}</button></header>${steps[draft.step - 1]}<footer class="modal-foot"><button class="btn btn-light" type="button" data-card-back>${draft.step === 1 ? "Cancelar" : "Voltar"}</button><button class="btn btn-primary" type="button" data-card-next>${draft.step === 3 ? "Criar cartão" : "Continuar"}</button></footer>`;
      ...
      host.querySelector("[data-card-next]").onclick = async (event) => {
        ...
        try {
          ...
          if (draft.step < 3) { draft.step++; renderCardWizard(); return; }
          event.currentTarget.disabled = true;
          const card = await window.FinancialSpaceService.createCreditCard(state.selectedSpaceId, { ...draft, limitCents: Engine.moneyInputToCents(draft.limit) });
          closeModal();
          Utils.toast("Cartão criado.");
  ...
  async function openCreditCardInvoice(invoiceId) {
    if (!invoiceId) return;
    try {
      const details = await window.FinancialSpaceService.getCreditCardInvoiceDetails(state.selectedSpaceId, invoiceId), invoice = details.invoice;
      sheet(`${sheetHeader(`Fatura ${invoice.cardName || "Cartão"}`, `${monthLabel(invoice.referenceKey)} · vence ${fullDateLabel(invoice.dueDate)}`)}<div class="modal-body"><article class="financial-invoice-summary"><div><small>Total da fatura</small><strong>${money(invoice.amountDueCents)}</strong></div><div><small>Pago</small><strong>${money(invoice.paidTotalCents)}</strong></div><div><small>Restante</small><strong>${money(invoice.remainingCents)}</strong></div><em class="financial-status is-${invoice.status === "paid" ? "paid" : invoice.status === "overdue" ? "overdue" : "pending"}">${esc(invoiceStatusLabel[invoice.status] || invoice.status)}</em></article><section class="financial-invoice-purchases"><h4>Compras</h4>${details.purchases.length ? details.purchases.map((purchase) => `<article><span><b>${esc(purchase.description)}</b><small>${dateLabel(purchase.purchaseDate)} · ${esc(purchase.categoryName || "Outros")}${purchase.installmentCount > 1 ? ` · ${purchase.installmentNumber}/${purchase.installmentCount}` : ""}</small></span><strong>${money(purchase.amountCents)}</strong></article>`).join("") : `<p>Nenhuma compra nesta fatura.</p>`}</section>${details.payments.length ? `<section class="financial-invoice-purchases"><h4>Pagamentos</h4>${details.payments.map((payment) => `<article><span><b>${esc(paymentLabel[payment.paymentMethod] || "Pagamento")}</b><small>${fullDateLabel(payment.paidAt)}</small></span><strong>${money(payment.amountCents)}</strong></article>`).join("")}</section>` : ""}</div><footer class="modal-foot"><button class="btn btn-light" type="button" data-financial-close>Fechar</button>${invoice.remainingCents > 0 ? `<button class="btn btn-primary" type="button" data-financial-pay-invoice="${esc(invoice.id)}">Pagar fatura</button>` : ""}</footer>`, "financial-invoice-sheet");
  ...
  async function openCreditCardInvoicePayment(invoice) {
    ...
    form.onsubmit = async (event) => {
      ...
      submit.disabled = true;
      try {
        await window.FinancialSpaceService.payCreditCardInvoice(state.selectedSpaceId, invoice.id, { ...values, operationId, amountCents, paidAt: new Date(`${values.paidAt}T12:00:00`).toISOString() });
        closeModal();
  ...
  async function openEntryForm(direction = "out") {
    ...
    const stepFour = () => {
      const category = selectedCategory(), subcategory = selectedSubcategory(), categoryName = draft.customCategoryName || category?.name || "—", subcategoryName = draft.customSubcategoryName || subcategory?.name || "Sem detalhar",
        card = cards.find((item) => item.id === draft.creditCardId), paymentDescription = draft.paymentChoice === "credit_card" ? `${card?.name || "Cartão"} · ${draft.installmentCount}x` : draft.paymentChoice === "pending" ? `Pendente · ${scheduleLabel()}` : paymentLabel[draft.paymentMethod] || "Pagamento imediato",
    ...
    const validateStep = () => {
      ...
      if (draft.step === 3) {
        if (draft.paymentChoice === "credit_card" && (!draft.creditCardId || !draft.purchaseDate)) throw new Error("Escolha o cartão e a data da compra.");
    ...
    const renderWizard = () => {
      ...
      host.querySelectorAll("[data-wizard-credit-card]").forEach((button) => button.onclick = () => { syncVisibleFields(); draft.creditCardId = button.dataset.wizardCreditCard; renderWizard(); });
      host.querySelector("[data-wizard-create-card]")?.addEventListener("click", () => { syncVisibleFields(); openCreateCreditCard(async (card) => { cards.push(card); draft.creditCardId = card.id; sheet(`<div data-financial-entry-wizard></div>`, "financial-entry-wizard"); host = modal().querySelector("[data-financial-entry-wizard]"); renderWizard(); }); });
  ...
  function bindPage() {
    ...
    page.querySelectorAll("[data-financial-open-invoice]").forEach((button) => button.onclick = (event) => { event.stopPropagation(); openCreditCardInvoice(button.dataset.financialOpenInvoice); });
    page.querySelectorAll("[data-financial-card-invoice]").forEach((button) => button.onclick = (event) => { event.stopPropagation(); openCreditCardInvoice(button.dataset.financialCardInvoice); });
    page.querySelectorAll("[data-financial-invoice-id]").forEach((row) => row.onclick = () => openCreditCardInvoice(row.dataset.financialInvoiceId));
