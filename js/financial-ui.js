window.FinanceiroUI = (() => {
  ...
  async function openEntryForm(direction = "out") {
    ...
    const syncVisibleFields = () => {
      ...
      for (const name of ["description", "amount", "customCategoryName", "customSubcategoryName", "frequency", "dueAt", "paymentMethod", "financialAccountId", "paidAt", "purchaseDate", "creditCardId", "notes", "categorySearch"])
    ...
    const renderWizard = () => {
      ...
      host.querySelector("[data-wizard-create-account]")?.addEventListener("click", () => { syncVisibleFields(); openCreateFinancialAccount(async (account) => { financialAccounts.push(account); draft.financialAccountId = account.id; sheet(`<div data-financial-entry-wizard></div>`, "financial-entry-wizard"); host = modal().querySelector("[data-financial-entry-wizard]"); renderWizard(); }); });
  ...
  async function openTransfer() {
    ...
    const loadDestinationAccounts = async () => {
      form.toFinancialAccountId.disabled = true;
      const available = form.toSpaceId.value === state.selectedSpaceId ? fromAccounts : await service.listFinancialAccounts(form.toSpaceId.value),
        accounts = form.toSpaceId.value === state.selectedSpaceId ? available.filter((account) => account.id !== form.fromFinancialAccountId.value) : available;
      form.toFinancialAccountId.innerHTML = accounts.length ? accounts.map((account) => `<option value="${esc(account.id)}">${esc(account.name)}</option>`).join("") : `<option value="">Nenhuma conta cadastrada</option>`;
      ...
      form.toFinancialAccountId.disabled = !accounts.length;
    };
    form.toSpaceId.addEventListener("change", () => loadDestinationAccounts().catch((error) => Utils.toast(error.message, true)));
    form.fromFinancialAccountId.addEventListener("change", () => loadDestinationAccounts().catch((error) => Utils.toast(error.message, true)));
    await loadDestinationAccounts();
    form.onsubmit = async (event) => { event.preventDefault(); const values = Object.fromEntries(new FormData(form)), submit = form.querySelector("[type=submit]"); submit.disabled = true; try { await service.createTransfer(state.selectedSpaceId, values.toSpaceId, { transferId, fromFinancialAccountId: values.fromFinancialAccountId, toFinancialAccountId: values.toFinancialAccountId, amountCents: Engine.moneyInputToCents(values.amount), occurredAt: new Date(`${values.occurredAt}T12:00:00`).toISOString(), description: values.description }); closeModal(); Utils.toast("Transferência registrada sem alterar o resultado."); await refresh(); } catch (error) { Utils.toast(error.message, true); submit.disabled = false; } };
  }
