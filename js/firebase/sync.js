...
let currentUser=null,lastError='',lastErrorCode='',timer=null,syncOperation=null,originalAlter=null,unsubscribers=[],subscribers=new Set(),applyingCloud=false,currentPath='';
const state={authReady:false,status:navigator.onLine?'idle':'offline',message:navigator.onLine?'Aguardando verificação da nuvem':'Offline — salvo no aparelho',progress:0,sent:{},failed:0,testPassed:false,lastSync:localStorage.getItem('adiFestaLastSync')||'',cloudCounts:{},activeListeners:0,userProfile:null,businessOwnerId:'',ownerMatches:false};
...
function reportError(error,context='Firestore operation',extra={}){lastError=friendlyError(error);lastErrorCode=errorCode(error)||'unknown';console.error(`[${context}]`,{...extra,code:error?.code,message:error?.message,stack:error?.stack});emit({status:'error',message:`Erro de sincronização — ${lastError}`,failed:state.failed+1});return lastError}
...
const diagnostics=()=>{const local=localSummary(),profile=state.userProfile||window.FirebaseSession?.profile||{};return{projectId:PROJECT_ID,databaseId:'(default)',authReady:state.authReady,authenticated:Boolean(auth.currentUser),uid:auth.currentUser?.uid||'',email:auth.currentUser?.email||'',userDocumentExists:Boolean(profile.uid||window.FirebaseSession?.profile),userActive:profile.active===true,userRole:profile.role||'',userBusinessId:profile.businessId||'',businessDocumentExists:Boolean(state.businessDocument),businessOwnerId:state.businessOwnerId||'',authenticatedUid:auth.currentUser?.uid||'',ownerMatches:state.ownerMatches===true,targetBusinessId:BUSINESS_ID,businessId:BUSINESS_ID,connection:state.testPassed?'funcionando':'não comprovada',activeListeners:state.activeListeners,localClients:local.clientes,cloudClients:state.cloudCounts.clients??'—',localProducts:local.produtos,cloudProducts:state.cloudCounts.products??'—',pendingOperations:readQueue().length,pending:readQueue().length,lastSyncAt:state.lastSync||'',lastSync:state.lastSync||'nunca',currentPath:currentPath||`businesses/${BUSINESS_ID}`,lastErrorCode,lastErrorMessage:lastError,lastError}};
...
async function validateUser(){
  ...
  const profile=snapshot.data();if(profile.uid!==user.uid)throw Object.assign(new Error('O UID do perfil não corresponde à sessão autenticada.'),{code:'permission-denied'});if(profile.active!==true)throw Object.assign(new Error('Sua conta está desativada.'),{code:'permission-denied'});if(profile.role!=='admin')throw Object.assign(new Error('Seu usuário não possui perfil de administrador.'),{code:'permission-denied'});if(profile.businessId!==BUSINESS_ID)throw Object.assign(new Error('Esta conta não possui acesso ao Adi Festa.'),{code:'permission-denied'});
  currentUser=user;state.userProfile=normalizeFirestoreData(profile);return{user,profile};
}
async function ensureBusinessDocument(){
  ...
  let confirmed=await getDoc(reference),business=confirmed.data()||{},ownerMatches=business.ownerId===user.uid;
  emit({businessDocument:confirmed.exists(),businessOwnerId:business.ownerId||'',ownerMatches});
  if(!confirmed.exists())throw Object.assign(new Error('O documento do negócio não foi encontrado.'),{code:'permission-denied'});
  if(!ownerMatches&&user.email==='murillo.charles97@gmail.com'&&state.userProfile?.role==='admin'){await setDoc(reference,{id:BUSINESS_ID,ownerId:user.uid,active:true,schemaVersion:Number(business.schemaVersion||2),ownershipRepairedAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});confirmed=await getDoc(reference);business=confirmed.data()||{};ownerMatches=business.ownerId===user.uid;emit({businessDocument:confirmed.exists(),businessOwnerId:business.ownerId||'',ownerMatches})}
  if(!ownerMatches)throw Object.assign(new Error('O proprietário do negócio no Firestore não corresponde ao usuário conectado.'),{code:'permission-denied'});if(business.active===false)throw Object.assign(new Error('O negócio Adi Festa está inativo.'),{code:'permission-denied'});
  if(business.active!==true||business.id!==BUSINESS_ID){await setDoc(reference,{id:BUSINESS_ID,active:true,schemaVersion:Number(business.schemaVersion||2),updatedAt:serverTimestamp()},{merge:true});confirmed=await getDoc(reference);business=confirmed.data()||{};emit({businessDocument:confirmed.exists(),businessOwnerId:business.ownerId||'',ownerMatches:business.ownerId===user.uid})}return reference;
}
async function testFirestoreConnection(){
  ...
  try{
    ...
    currentPath='';lastError='';lastErrorCode='';sessionStorage.setItem('adiFirebaseConnectionOk','true');emit({status:'success',message:'Conexão com Firestore funcionando.',testPassed:true});return normalizeFirestoreData(data);
...
async function synchronizeNow(){
  ...
  syncOperation=(async()=>{try{await validateUser();await testFirestoreConnection();const push=await pushPendingOperations();await pullCloudCollections();const comparison=await compareLocalAndCloud(),time=new Date().toISOString();await setDoc(doc(db,'businesses',BUSINESS_ID,'syncMetadata','last-sync'),{id:'last-sync',businessId:BUSINESS_ID,ownerId:currentUser.uid,projectId:PROJECT_ID,status:comparison.ok?'ok':'divergent',comparison:sanitizeForFirestore(comparison),pendingOperations:readQueue().length,syncedAt:serverTimestamp(),updatedAt:serverTimestamp()},{merge:true});localStorage.setItem('adiFestaLastSync',time);emit({status:'success',message:`Sincronizado às ${new Date(time).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}`,progress:100,lastSync:time});return{push,comparison}}catch(error){if(state.status!=='error')reportError(error,'Synchronize now');throw error}})();
...
async function runFirebaseDiagnostic(){await validateUser();await ensureBusinessDocument();await testFirestoreConnection();return diagnostics()}
function setUser(user,profile=null){currentUser=user||null;emit({authReady:true,userProfile:profile||null});if(!user){stopCloudSubscriptions();emit({testPassed:false,businessDocument:false,businessOwnerId:'',ownerMatches:false,status:'idle',message:'Sessão encerrada'});return}installHybridStorage();state.testPassed=false;sessionStorage.removeItem('adiFirebaseConnectionOk');startCloudSubscriptions();if(readQueue().length)schedule()}
...
window.dataRepository={clients:repositories.clients,products:repositories.products,sales:repositories.sales,payments:repositories.payments,balanceAdjustments:repositories.balanceAdjustments,stockMovements:repositories.stockMovements,campaigns:repositories.campaigns,campaignProgress:repositories.campaignProgress,rewards:repositories.rewards,charges:repositories.charges,settings:repositories.settings};
window.getFirebaseDiagnostic=diagnostics;
window.SyncFirebase={setUser,setAuthReady:value=>emit({authReady:Boolean(value)}),stop:()=>setUser(null),subscribe:callback=>{subscribers.add(callback);callback({...state,details:diagnostics()});return()=>subscribers.delete(callback)},sanitizeForFirestore,ensureBusinessDocument,testFirestoreConnection,testConnection:testFirestoreConnection,runFirebaseDiagnostic,getFirebaseDiagnostic:diagnostics,startCloudMigration,processSyncQueue:synchronizeNow,synchronizeNow,syncAll:synchronizeNow,migrate:startCloudMigration,pushPendingOperations,pullCloudCollections,compare:compareLocalAndCloud,startCloudSubscriptions,stopCloudSubscriptions,schedule,snapshot:localSummary,diagnostics,isReady:()=>Boolean(currentUser)};
