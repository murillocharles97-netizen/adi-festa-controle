window.Vendas=(()=>{
  ...
  const registrar=d=>{const operationId=d.operationId||Utils.uuid(),existente=DB.carregar().vendas.find(v=>v.operationId===operationId);if(existente)return existente;if(window.PlanLimitService)PlanLimitService.assert(PlanLimitService.canCreateSale(),'registrar novas vendas');let criada;DB.alterar(db=>{
