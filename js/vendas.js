window.Vendas=(()=>{
  ...
  const registrar=d=>{let criada;DB.alterar(db=>{
    ...
    criada={id:Utils.uuid(),clienteId:d.clienteId||null,clienteNome:cliente?.nome||'Venda avulsa',itens,subtotalOriginal,descontoTotal,valorFinal,valorTotal:valorFinal,custoTotal,lucro,status:d.status,data,observacao:d.observacao||'',saldoAnterior,saldoAtual,ajusteManual:Boolean(d.ajusteManual),descontoTipo:d.descontoTipo||null};
