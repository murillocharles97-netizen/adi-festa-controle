(() => {
  ...
  function render() {
    ...
    return `<section class="desktop-settings settings" data-desktop-settings>
      <div class="desktop-settings-grid">
        ...
        ${card("puzzle", "Integrações e maquininhas", "Conecte terminais e serviços ao caixa.", `<ul class="desktop-integration-list"><li>${icon("credit-card")}<span>Cielo</span><em>Não configurado</em></li><li>${icon("badge-dollar-sign")}<span>Mercado Pago</span><em>Não configurado</em></li><li>${icon("landmark")}<span>PagBank</span><em>Em breve</em></li></ul><button class="btn btn-light" type="button" data-terminal-settings>Gerenciar maquininhas</button>`)}
  ...
  function bind() {
    ...
    root.querySelector("[data-terminal-settings]")?.addEventListener("click", () =>
      window.TerminalPayments?.openSettings?.(),
    );
    root
