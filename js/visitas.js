window.Visitas=(()=>{
  ...
  function converter(id,saleStatus='pago'){
    ...
    const operationId=`catalog-order:${id}`,data=DB.carregar(),sale=Vendas.registrar({operationId,clienteId:cliente?.id||null,status:saleStatus,formaPagamento:existing.paymentPreference,observacao:`Pedido online #${existing.publicOrderNumber}${existing.note?` — ${existing.note}`:''}`,itens:existing.items.map(item=>{const product=data.produtos.find(produto=>produto.id===item.productId),variant=item.variantId?(data.variacoesProdutos||[]).find(entry=>entry.id===item.variantId):null;return{produtoId:item.productId,variantId:item.variantId||null,nome:item.name,productNameSnapshot:item.productNameSnapshot||product?.nome,variantNameSnapshot:item.variantNameSnapshot||variant?.displayName||null,attributesSnapshot:item.attributesSnapshot||variant?.attributeValues||null,sku:item.sku||variant?.sku||'',barcode:item.barcode||variant?.barcode||'',quantidade:item.quantity,precoOriginal:item.unitPrice,precoFinalUnitario:item.unitPrice,custoUnitario:Number(variant?.cost??product?.custo??0)}})});
...
window.VisitasUI=(()=>{
  ...
  let onlineOrderFilter='todos',onlineOrderQuery='',onlineOrderLimit=50;
  ...
  function renderOrdersPage(){
    const all=DB.carregar().catalogOrders||[],received=all.filter(order=>order.orderStatus==='recebido').length,list=onlineOrders(),shown=list.slice(0,onlineOrderLimit);
    return `<section class="page-head online-orders-head"><div><span class="eyebrow">Catálogo online</span><h2>Pedidos online ${received?`<span class="new-order-badge">${received}</span>`:''}</h2><p>Receba, vincule e acompanhe os pedidos enviados pelos clientes.</p></div></section>
      ...
      <section class="order-list online-orders-list">${shown.map(onlineOrderCard).join('')||'<div class="visit-empty"><p>Nenhum pedido encontrado.</p></div>'}</section>
      ${shown.length<list.length?`<div class="modal-foot"><button class="btn btn-light" data-load-online-orders>Carregar mais pedidos</button></div>`:''}`;
  }
  function bindOrdersPage(){
    const filter=document.querySelector('#online-order-filter');if(filter)filter.value=onlineOrderFilter;
    filter?.addEventListener('change',event=>{onlineOrderFilter=event.target.value;onlineOrderLimit=50;dispatchEvent(new HashChangeEvent('hashchange'))});
    document.querySelector('#online-order-search')?.addEventListener('input',event=>{onlineOrderQuery=event.target.value.trim().toLowerCase();onlineOrderLimit=50;const list=document.querySelector('.online-orders-list');if(list){list.innerHTML=onlineOrders().slice(0,onlineOrderLimit).map(onlineOrderCard).join('')||'<div class="visit-empty"><p>Nenhum pedido encontrado.</p></div>';window.lucide?.createIcons()}});
    document.querySelector('[data-load-online-orders]')?.addEventListener('click',()=>{onlineOrderLimit+=50;dispatchEvent(new HashChangeEvent('hashchange'))});
