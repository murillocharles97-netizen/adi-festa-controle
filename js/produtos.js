...
window.Produtos=(()=>{
  ...
  const excluir=id=>{const result=DB.alterar(db=>db.produtos=db.produtos.filter(p=>p.id!==id));window.BarcodeIndex?.invalidate?.();return result};
