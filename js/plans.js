(function(){
  ...
  function couponSummary(quote){return`<div class="coupon-applied">${icon('badge-check')}<div><b>Cupom ${esc(quote.code)} aplicado</b><span>${esc(plans().find(plan=>plan.id===quote.planId)?.name||quote.planId)} · ${quote.billingCycle==='yearly'?'anual':'mensal'}</span><p>De <s>${money(quote.originalPrice)}</s> por <strong>${money(quote.discountedPrice)}</strong></p><small>${esc(quote.durationLabel)}.</small></div><button type="button" data-remove-coupon>Remover cupom</button></div>`}
  function couponBox(available,view){
    if(view.publicMode||view.internal)return'';const selectedPlan=couponQuote?.planId||'professional',selectedCycle=couponQuote?.billingCycle||'monthly';
    return`<details class="plan-coupon-box" data-coupon-box ${couponQuote?'open':''}><summary>${icon('ticket-percent')}<span><b>Possui um cupom de desconto?</b><small>A validação segura acontece somente ao tocar em Aplicar.</small></span>${icon('chevron-down')}</summary><div class="plan-coupon-body"><div class="plan-coupon-fields"><label>Plano<select data-coupon-plan>${available.map(plan=>`<option value="${plan.id}" ${plan.id===selectedPlan?'selected':''}>${esc(plan.name)}</option>`).join('')}</select></label><label>Periodicidade<select data-coupon-cycle><option value="monthly" ${selectedCycle==='monthly'?'selected':''}>Mensal</option><option value="yearly" ${selectedCycle==='yearly'?'selected':''}>Anual</option></select></label><label class="coupon-code-field">Código<input data-coupon-code autocomplete="off" maxlength="40" placeholder="Digite o código" value="${esc(couponQuote?.code||'')}"></label><button class="btn btn-primary" type="button" data-apply-coupon>Aplicar</button></div><div class="coupon-feedback" data-coupon-feedback aria-live="polite">${couponQuote?couponSummary(couponQuote):''}</div></div></details>`
  }
  ...
  function bind(root=document,bindOptions={}){
    ...
    $$('[data-plan-cta]',scope).forEach(button=>button.onclick=async()=>{if(options.publicMode)return options.onRegister?.();button.disabled=true;try{const planId=button.dataset.planCta,billingCycle=$('[data-coupon-cycle]',scope)?.value||'monthly',quote=couponQuote?.planId===planId&&couponQuote?.billingCycle===billingCycle?couponQuote:null,result=await window.SubscriptionService?.requestUpgrade?.(planId,{billingCycle,quoteId:quote?.quoteId||null});if(result?.checkoutUrl){location.assign(result.checkoutUrl);return}window.Utils?.toast?.(result?.message||'Não foi possível abrir o checkout.',true)}catch(error){window.Utils?.toast?.(error.message||'Não foi possível iniciar a assinatura.',true)}finally{button.disabled=false}});
    const bindCouponRemoval=root=>{$('[data-remove-coupon]',root)?.addEventListener('click',()=>{couponQuote=null;const feedback=$('[data-coupon-feedback]',scope);if(feedback)feedback.innerHTML='';const input=$('[data-coupon-code]',scope);if(input)input.value='';window.Utils?.toast?.('Cupom removido.')})};bindCouponRemoval(scope);
    $('[data-apply-coupon]',scope)?.addEventListener('click',async event=>{const button=event.currentTarget,code=$('[data-coupon-code]',scope)?.value?.trim(),planId=$('[data-coupon-plan]',scope)?.value,billingCycle=$('[data-coupon-cycle]',scope)?.value,feedback=$('[data-coupon-feedback]',scope);if(!code){feedback.textContent='Digite um código de cupom.';return}button.disabled=true;button.textContent='Validando…';feedback.innerHTML='<span class="coupon-validating">Validando a condição com segurança…</span>';try{couponQuote=await window.SubscriptionService.validateCoupon({couponCode:code,planId,billingCycle});feedback.innerHTML=couponSummary(couponQuote);bindCouponRemoval(feedback);window.Utils?.toast?.('Cupom aplicado com segurança.')}catch(error){couponQuote=null;feedback.innerHTML=`<span class="coupon-error">${icon('circle-alert')} ${esc(error.message||'Não foi possível validar o cupom agora. Tente novamente.')}</span>`}finally{button.disabled=false;button.textContent='Aplicar';window.lucide?.createIcons()}});
    $$('[data-coupon-plan],[data-coupon-cycle]',scope).forEach(control=>control.addEventListener('change',()=>{if(couponQuote){couponQuote=null;const feedback=$('[data-coupon-feedback]',scope);if(feedback)feedback.innerHTML='<span class="coupon-warning">Plano ou periodicidade alterados. Aplique o cupom novamente.</span>'}}));
