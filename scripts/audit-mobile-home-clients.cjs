...
async function main(){
  ...
  try{
    ...
    for(const [width,height] of VIEWPORTS){
      ...
      const emptyAttention=await evaluate(cdp,`(()=>{window.__auditData.produtos.forEach(item=>{item.estoqueAtual=20;item.estoqueMinimo=1});window.__auditData.clientes.forEach(item=>item.saldo=0);window.__auditData.customerSubscriptions=[];window.__auditData.catalogOrders=[];window.__renderAudit('inicio');return document.querySelector('.home-attention').textContent})()`);if(!/Tudo em dia por aqui/.test(emptyAttention))throw Error(`${width}x${height}: estado vazio de atenção ausente`);
