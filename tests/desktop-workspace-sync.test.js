...
test("publica os arquivos desktop e o identificador do build em cache novo", () => {
  ...
  assert.match(index, /build-info\.js\?v=151/);
  assert.match(index, /desktop-sales\.css\?v=140/);
  assert.match(index, /checkout\.js\?v=151/);
  ...
  assert.match(worker, /release:'151'/);
  assert.match(build, /release: "151"/);
  assert.match(index, /firebase-ui\.js\?v=151/);
