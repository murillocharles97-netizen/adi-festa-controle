...
const saleDraft=id=>({id,clienteId:null,observacao:'Teste presencial',itens:[{produtoId:'product-1',nome:'Produto teste',quantidade:1,precoOriginal:89.9,precoFinalUnitario:89.9,custoUnitario:20}]});
...
(async()=>{
  ...
  const approvedPayload={businessId:businessA.businessId,idempotencyKey:'approved_attempt_0001',terminalId:terminal.id,amountCents:8990,paymentMethod:'credit',installments:2,simulatorScenario:'approved',saleDraft:saleDraft('sale_approved_0001')};
  ...
  const declined=(await createPayment({...approvedPayload,idempotencyKey:'declined_attempt_0001',simulatorScenario:'declined',saleDraft:saleDraft('sale_declined_0001')})).data.intent;
  ...
  const timeoutPayload={...approvedPayload,idempotencyKey:'timeout_attempt_000001',simulatorScenario:'timeout',saleDraft:saleDraft('sale_timeout_00001')},timeout=(await createPayment(timeoutPayload)).data.intent;
  ...
  await assert.rejects(()=>createPayment({...approvedPayload,businessId:businessB.businessId,idempotencyKey:'cross_business_000001',saleDraft:saleDraft('sale_cross_000001')}),error=>['functions/not-found','functions/permission-denied'].includes(error.code));
